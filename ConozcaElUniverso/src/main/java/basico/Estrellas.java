...
public class Estrellas extends CuerposCelestes{
    ...
    public void pregunta(Estrellas X,Estrellas Y){
        ...
        //Opciones para aceptar saber que clase de respuesta aceptar
            switch (a){
            case 0 -> lecRes(constelacion,X.getConstelacion(),Y.getConstelacion());
            case 1 -> lecRes(distanciaTierra, X.getDistanciaTierra(),Y.getDistanciaTierra());
            case 2 -> lecRes(tipo,X.getTipo(),Y.getTipo());
            case 3 -> lecRes(luminosidad,X.getluminosidad(),Y.getluminosidad());
        }
